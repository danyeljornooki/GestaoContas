document.addEventListener("DOMContentLoaded", function () {
  ...
  // Funções relacionadas ao carregamento e manipulação das contas
  let todasContas = JSON.parse(localStorage.getItem("todasContas")) || [];

  // Funções relacionadas ao Dashboard e carregamento de conteúdo
  ...
  // Objeto com os meses e seus equivalentes numéricos
  ...
  function carregarContas(selecionado) {
    conteudoDashboard.style = "display: block";
          ...
          if (
            confirm(
              `Tem certeza que deseja excluir ${
                contasSelecionadas.length === 1 ? "esta conta" : "estas contas"
              }?`
            )
          ) {
                ...
                .map(
                  (conta) => `
                      <tr>
                          ...
                          <td>
                              <button class="editar-btn" data-id="${
                                conta.id
                              }">Editar</button>
                          </td>
                      </tr>
  ...
  // Funções relacionadas ao formulário de adição de conta
  ...
  // Adiciona listeners de clique aos botões do dashboard
  botoesDashboard.forEach((btn) => {
    btn.addEventListener("click", function () {
      limparConteudoDashboard();
